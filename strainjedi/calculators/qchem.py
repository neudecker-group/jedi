"""ASE glue for driving Q-Chem: the calculator, its input writer and optimisation dynamics.

This is calculator plumbing, not parsing -- see :mod:`strainjedi.io` for the latter. It stays
here rather than moving upstream because the input writer emits ``$distort``, ``PHESS``/
``N_SOL`` and ``$alist`` blocks for CoGEF and partial-Hessian jobs, which stock ASE cannot.
"""
...
class QChem(qchem.QChem):
    ...
    def get_vibrations(self, atoms):
        """Read the Hessian this calculator's job produced.

        Delegates to the parser layer, which un-weights Q-Chem's mass-weighted Hessian using
        the masses Q-Chem itself reports.
        """
        return to_vibrations(read_output(f"{self.label}.out"), atoms)
