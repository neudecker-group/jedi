...
class ColorMapper:
    ...
    def check_bond_in_angle(self, bond, angle):
        return ((angle[0] == bond[0] and angle[1] == bond[1])
    ...
    def check_bond_in_dihedral(self, bond, dihedral):
        return ((dihedral[0] == bond[0] and dihedral[1] == bond[1])
    ...
    def handle_pbc(self, E_array, n_regular, n_custom):
        ...
        for i in range(len(atoms_ex_cell)):
            ...
            is_regular = (n_regular > 0 and
                          len(np.where(np.all(original_rim == bondscheck, axis=1))[0]) > 0)
            is_custom = (n_custom > 0 and
                         len(np.where(np.all(original_rim == customcheck, axis=1))[0]) > 0)
    ...
    def generate_colors(self, colormap, n_colors):
        ...
        if isinstance(colormap, Colormap):
            ...
            for i in range(n_colors):
                ...
                G = min(1.0, 2 - float(i + 1) / (n_colors / 2)) if n_colors % 2 == 0 else min(1.0, 2 - float(i) / (n_colors / 2))
            ...
            raise ValueError(f"Unknown color scheme: {colormap}. "
