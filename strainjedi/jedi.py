...
class JediAtoms(Jedi):
    ...
    def get_delta_x(self):
        ...
        if self.atomsF.get_pbc().any() is False:
            minimised_atomsF = kabsch_fit(self.atomsF.positions,self.atoms0.positions)  # to get rid of strain for translation and rotation
...
''')
        ...
        if pbc_flag == True:
            ...
            for i in range(len(atoms_ex_cell)):
                ...
                if len(np.where(np.all(original_rim == bondscheck, axis=1))[0]) > 0:
                    ...
                    E_array_pbc = np.append(E_array_pbc, [[ex_ind], [E_array_value]],axis=1)  # add to bond list with auxillary index
    ...
    def pov_gen(self,colorbar=True,box=False,man_strain=None,label='pov',incl_coloring=None,view_dir=None,zoom=1.,tex='vmd',
        radii=1.,scale_radii=None,bond_colors=None,cameratype='perspective',cameralocation=(0., 0., 20.),
        look_at=(0., 0., 0.),camera_right_up=[(-8., 0., 0.), (0., 6., 0.)],cameradirection=(0., 0., 10.),light=None,
        ...
        if pbc_flag == True:
            ...
            for i in range(len(atoms_ex_cell)):
                pos_ex_atom = atoms_f.get_positions()[int(atoms_ex_cell[i, 0])] + atoms_ex_cell[i,5:8]  # get positions of cell external atoms by adding the vector
                # if pos_ex_atom in mol.positions:
                original_rim = [int(atoms_ex_cell[i, 0]),int(atoms_ex_cell[i, 1])]  # get the indices of the corresponding atoms inside the cell
                original_rim.sort()  # needs to be sorted because rim list only covers one direction
                if len(np.where(np.all(atoms_f.positions == pos_ex_atom, axis=1))[0]) > 0:
                    ...
                    ex_ind = len(atoms_f)
                    if len(np.where(np.all(original_rim == bondscheck, axis=1))[0]) > 0:
                        atoms_f.append(Atom(symbol=atoms_f.symbols[int(atoms_ex_cell[i, 1])],position=pos_ex_atom))  # append to the virtual atoms object
